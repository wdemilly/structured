...
APP_VERSION = "v2.3 · Claude API + Pangram"
...
def build_claude_test_message(prompt_text: str, packet_text: str, target_words: int) -> str:
    ...
    return (
        ...
        + f"Draft the chapter now. For this rapid test, aim for approximately {int(target_words)} words. "
          "Return only the finished fiction prose: no preface, explanation, analysis, checklist, or notes. "
          "The application will create the DOCX download.\n"
        + "</rapid_test_harness>"
    )
...
def run_claude_stream(
    ...
    *,
    ...
    # Current Claude 5 models think by default. For rapid mechanical prose tests,
    # explicitly disabling thinking removes that latency while keeping every run
    # on the same controlled setting. Older Claude 4.x models already default to
    # no thinking when the field is omitted.
    if thinking_mode == "No thinking (fast)":
        if re.search(r"(?:^|-)5(?:$|-)", model):
    ...
    return final_text, final_message, elapsed
...
with claude_tab:
    ...
    with c1:
        run_label = st.text_input("Run label", value="rapid-test", key="claude_run_label_v23")
    with c2:
        target_words = st.number_input(
            ...
            step=50,
            key=f"claude_target_words_v23_{target_default}",
        )
    with c3:
        thinking_mode = st.selectbox(
            "Claude reasoning",
            ["No thinking (fast)", "Adaptive thinking"],
            index=0,
            key="claude_thinking_v23",
            help="No thinking is the controlled low-latency default for these mechanical prose experiments.",
        )
    with c4:
    ...
    if st.button(
        ...
        live = st.empty()
        run_status = st.status("Claude generation running…", state="running")
        try:
            output_text, final_message, generation_seconds = run_claude_stream(
                anthropic_client,
                anthropic_model,
                exact_payload,
                max_tokens=int(max_tokens),
    ...
    if last:
        ...
        with st.expander("Generated prose", expanded=True):
            st.text_area("Claude output", value=last["output_text"], height=420, disabled=True, key="claude_output_v23")
        st.download_button(
            "Download Claude output as DOCX",
            data=prose_docx_bytes(last["output_text"]),
            file_name=f"{(rec.get('label') or 'rapid-test').replace(' ', '_')}_Claude.docx",
            mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            key="claude_docx_download_v23",
        )
        ...
        st.download_button(
            ...
            file_name=f"{(rec.get('label') or 'rapid-test').replace(' ', '_')}_run.json",
            mime="application/json",
            key="claude_json_download_v23",
        )
    ...
    if not recent_generations.empty:
        with st.expander("Recent Claude → Pangram runs", expanded=False):
            display_cols = [
                "run_at", "label", "anthropic_model", "thinking_mode", "target_words", "output_words",
