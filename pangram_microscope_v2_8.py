...
APP_VERSION = "v2.8 · v2.5 connection base · explicit model + effort"
...
def run_claude_stream(
    ...
    *,
    ...
    # Claude Opus 5 and Fable 5 use adaptive thinking by default.  Do not send
    # thinking={"type":"disabled"}; that was the key mismatch in v2.5.
    # Effort is the supported control for how much test-time compute they use.
...
with claude_tab:
    ...
    # IMPORTANT: v2.8 is rebuilt directly from the known-working v2.5 connection
    # code.  The model selector here is only a generation control; it does not
    # change how the Anthropic client connects.
    ...
    with g1:
        if tab_models:
            model_index = tab_models.index(current_model) if current_model in tab_models else 0
            selected_model = st.selectbox(
                "Claude model",
                tab_models,
                index=model_index,
                key="claude_model_tab_v28",
                help="Exact model ID returned by Anthropic's Models API. This choice is used for the generation request.",
            )
            st.session_state["anthropic_model"] = selected_model
        else:
            selected_model = None
            st.selectbox("Claude model", ["Connect Anthropic first"], disabled=True, key="claude_model_tab_disabled_v28")
    with g2:
        effort = st.selectbox(
            "Claude effort",
            CLAUDE_EFFORT_LEVELS,
            index=CLAUDE_EFFORT_LEVELS.index(CLAUDE_DEFAULT_EFFORT),
            key="claude_effort_v28",
    ...
    with g3:
        max_tokens = st.number_input(
            ...
            step=512,
            key="claude_max_tokens_v28",
        )

    r1, r2 = st.columns(2)
    with r1:
        run_label = st.text_input("Run label", value="rapid-test", key="claude_run_label_v28")
    with r2:
        target_words = st.number_input(
            ...
            step=50,
            key=f"claude_target_words_v28_{target_default}",
        )
    ...
    anthropic_client, sidebar_model = get_connected_anthropic()
    anthropic_model = selected_model or sidebar_model
    if anthropic_model:
        ...
        st.info(f"Claude request: **{anthropic_model}** · **{mode_note}**")
