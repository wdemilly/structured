...
APP_VERSION = "v2.15 · claude.ai writing replication"
...
def save_generation_run(record: dict[str, Any]) -> None:
    ...
    cols = [
        ...
        "turn_index", "chapter_source", "chapter_filename", "reply_text",
    ]
    placeholders = ",".join(["?"] * len(cols))
    values = [record.get(c) for c in cols]
    with sqlite3.connect(DB_PATH) as con:
    ...
    if not recent.empty:
        with st.expander("Recent writing runs", expanded=False):
            display_cols = [
                ...
                "system_prompt_fingerprint", "chapter_source", "output_words",
