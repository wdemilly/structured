...
APP_VERSION = "v2.23 · prose in the reply (2.17 base, no sandbox)"
...
# Opus 5 and Sonnet 5 top out at 128k output tokens. v2.17 let this input go to
# 200,000, which the API rejects. max_tokens also covers thinking, so at xhigh or
# max effort a low cap can be spent entirely on reasoning with no prose left --
# that is the run that returned zero words.
...
# The drafting prompts say "return the chapter as a Word .docx file". No tools are
# sent, so there is nowhere to put one. v2.14 handled this by telling Claude the
# harness would build the file; the wording below is v2.14's, unchanged, because
# that is the configuration that wrote chapters. The app still produces the .docx
# download -- prose_docx_bytes() builds it from the returned prose.
DELIVERY_OVERRIDE = (
...
def build_web_style_user_turn(attachments: list[Attachment], user_message: str) -> str:
    """Documents first, the line the writer typed, then the delivery override."""
    documents = build_documents_block(attachments)
    instruction = (user_message or "").strip()
    instruction = (instruction + "\n\n" + DELIVERY_OVERRIDE).strip() if instruction else DELIVERY_OVERRIDE
    if documents:
        return documents + "\n\n" + instruction
    return instruction
...
def run_web_style_turn(
    ...
    *,
    ...
    for attempt in range(1, CLAUDE_MAX_TOOL_CONTINUATIONS + 1):
        ...
        # No tools. The chapter is the reply, so text_stream yields it token by
        # token as soon as the thinking block closes. Sending code_execution moves
        # composition inside a tool call, where nothing streams and a stall is
...
with claude_tab:
    ...
    # =====================================================================
        ...
        message_template = st.text_input(
            ...
            key="write_message_template_v217",
            help="{chapter} is substituted from the Chapter box above. Edit this to whatever you actually type in the app.",
        )
        try:
