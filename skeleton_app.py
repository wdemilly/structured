"""
Skeleton Extractor — Streamlit app
==================================

Builds a Structural Transplant skeleton (v6F format) from one or more donor
documents.

Deploy on Streamlit Community Cloud:
    1. Put this file and requirements.txt in a GitHub repo (any branch).
    2. share.streamlit.io -> New app -> pick the repo, branch, and
       streamlit_app.py as the main file.
    3. Deploy. No secrets, no config, no data files are needed.

Reads .docx, .txt and .md. Legacy .doc is not supported — save as .docx first.

Nothing is written to disk: uploads are held in memory and output is returned
through download buttons, so the app works on Cloud's ephemeral filesystem and
leaves no donor text behind between sessions.

The tagger is deterministic and rule-based: the same donor always produces the
same skeleton. It uses no external model, so no download is required and no run
differs from the last. Its known limits are listed in the app under ACCURACY.
"""
...
def paragraphs(t):
    return [p.strip() for p in normalise(t).split("\n") if p.strip()]
...
def build_units(paras):
    ...
    for p in paras:
        ...
        blocks.append(dict(units=units, frame=" ".join(frame_sentence(u["toks"], u["tags"]) for u in units)))
    return blocks
...
def render(blocks, order, include_frame, donor_note, seed):
...
def main():
    ...
    stl.title("Skeleton Extractor")
    stl.caption("Builds a Structural Transplant skeleton in v6F format from one or more donor documents.")
    ...
    if not files:
        stl.info("Upload one or more documents to begin. Multiple files are joined in the order you set below, "
                 "and treated as a single contiguous donor.")
        return
    ...
    for n in order_names:
        try:
            texts.append(read_upload(n, by_name[n].getvalue()))
    ...
    stl.write("**Loaded:** %d document(s), %d paragraphs, %d words." % (len(texts), len(paras), total_words))

    c1, c2, c3 = stl.columns(3)
    with c1:
        use_window = stl.checkbox("Use a window rather than the whole text", value=False)
        target = stl.number_input("Window size (words)", 500, 20000, 4800, 100, disabled=not use_window)
    with c2:
