...
APP_VERSION = "v2.7 · model selector in Claude tab · effort · adaptive thinking"
...
def preferred_anthropic_model(models: list[str]) -> str | None:
    # Keep Opus 5 as the continuity default, but expose every model returned by
    # Anthropic's Models API. Fable 5 can be selected explicitly in the UI.
    for preferred in (
        "claude-opus-5",
        "claude-fable-5",
        "claude-sonnet-5",
...
def model_supports_adaptive_thinking(model: str) -> bool:
    """Return True for current Claude models that use adaptive thinking.

    Anthropic's current API uses adaptive thinking on Claude 4.6+ and Claude 5
    families. Fable 5 / Mythos 5 always think; Opus 5 / Sonnet 5 think by
    default. We send adaptive explicitly so the API run is auditable and as
    close as possible to the current Claude web-app behavior.
    """
    m = (model or "").lower()
    if any(tag in m for tag in ("claude-fable-5", "claude-mythos-5", "claude-opus-5", "claude-sonnet-5")):
        return True
    if any(tag in m for tag in ("claude-opus-4-8", "claude-opus-4-7", "claude-opus-4-6", "claude-sonnet-4-6")):
        return True
    if "mythos-preview" in m:
        return True
    return False


def model_supports_effort(model: str) -> bool:
    """Return True for model families Anthropic documents as effort-capable."""
    m = (model or "").lower()
    if model_supports_adaptive_thinking(m):
        return True
...
def run_claude_stream(
    ...
    *,
    ...
    # Match the current Claude product family rather than the old test harness's
    # forced no-thinking mode. For Claude 4.6+ / Claude 5 models, adaptive
    # thinking is explicit and effort controls how much work the model does.
    if model_supports_adaptive_thinking(model):
        kwargs["thinking"] = {"type": "adaptive"}
        thinking_mode = "Adaptive"
    else:
        # Older models have different thinking controls; leave their model
        # default untouched instead of sending an incompatible adaptive value.
        thinking_mode = "Model default"

    if model_supports_effort(model):
        kwargs["output_config"] = {"effort": str(effort)}
        effort_applied = str(effort)
    else:
        effort_applied = "not supported / omitted"
    ...
    return final_text, final_message, elapsed, thinking_mode, effort_applied
...
def init_db() -> None:
    with sqlite3.connect(DB_PATH) as con:
        ...
        _ensure_column(con, "generation_runs", "effort", "TEXT")
        con.commit()
...
def save_generation_run(record: dict[str, Any]) -> None:
    init_db()
    cols = [
        "run_id", "run_at", "label", "anthropic_model", "thinking_mode", "effort",
...
with st.sidebar:
    ...
    if amodels:
        st.caption(f"{len(amodels)} Claude API model(s) loaded. Choose the model in Tab 1 · Claude → Pangram.")
...
with claude_tab:
    ...
    st.write(
        "Upload the drafting prompt and the rapid-test chapter packet. Each click sends one brand-new Messages API "
        "request containing only those materials, streams the prose as Claude writes it, then sends the finished prose "
        "straight to Pangram. Prior Claude outputs and prior Pangram scores are never included in the generation request."
    )
    st.info(
    ...
    # Put model selection in the working tab, next to effort, so the exact
    # generation configuration is visible without scrolling the sidebar.
    if amodels:
        preferred_order = [
            "claude-fable-5",
            "claude-opus-5",
        ...
        ordered_models = [m for m in preferred_order if m in amodels] + [m for m in amodels if m not in preferred_order]
        current_model = st.session_state.get("anthropic_model")
        if current_model not in ordered_models:
            current_model = preferred_anthropic_model(ordered_models)
        model_index = ordered_models.index(current_model) if current_model in ordered_models else 0
    else:
        ordered_models = []
        model_index = 0
    ...
    def _friendly_model_label(model_id: str) -> str:
        ...
        friendly = labels.get(model_id)
        return f"{friendly}  ·  {model_id}" if friendly else model_id

    gset1, gset2, gset3 = st.columns([2.0, 1.0, 1.0])
    with gset1:
        if ordered_models:
            selected_model = st.selectbox(
                ...
                ordered_models,
                index=model_index,
                format_func=_friendly_model_label,
                key="claude_model_v27",
                help="Exact Anthropic API model ID. This selection is sent as the model= parameter on every generation request.",
            )
            st.session_state["anthropic_model"] = selected_model
        else:
            selected_model = None
            st.selectbox("Claude model", ["Connect Anthropic first"], disabled=True, key="claude_model_disabled_v27")
    with gset2:
        effort = st.selectbox(
            "Claude effort",
            CLAUDE_EFFORT_LEVELS,
            index=CLAUDE_EFFORT_LEVELS.index(CLAUDE_DEFAULT_EFFORT),
            key="claude_effort_v27",
            help="Anthropic API effort. High is the API default; lower/higher levels trade latency and cost for reasoning depth.",
        )
    with gset3:
        max_tokens = st.number_input(
            ...
            step=512,
            key="claude_max_tokens_v27",
            help="This limit includes thinking plus visible response on current adaptive-thinking models.",
        )

    rset1, rset2 = st.columns(2)
    with rset1:
        run_label = st.text_input("Run label", value="rapid-test", key="claude_run_label_v27")
    with rset2:
        target_words = st.number_input(
            ...
            step=50,
            key=f"claude_target_words_v27_{target_default}",
        )

    exact_payload = build_claude_test_message(prompt_text, packet_text, int(target_words)) if prompt_text and packet_text else ""
    if exact_payload:
        with st.expander("Exact user message Claude will receive", expanded=False):
            st.text_area("API payload", value=exact_payload, height=420, disabled=True, key="claude_payload_preview_v23")
            st.caption("The app does not append prior conversation turns, prior outputs, or detector feedback.")

    pangram_client, pangram_model = get_connected_client()
    anthropic_client, anthropic_model = get_connected_anthropic()
    if anthropic_model:
        adaptive_note = "Adaptive thinking" if model_supports_adaptive_thinking(anthropic_model) else "Model-default thinking"
        effort_note = effort if model_supports_effort(anthropic_model) else "effort omitted (unsupported)"
        st.info(f"Claude request: **{anthropic_model}** · **{adaptive_note}** · effort **{effort_note}**")
    ready = bool(prompt_text.strip() and packet_text.strip() and anthropic_client and anthropic_model and pangram_client and pangram_model)
    ...
    if st.button(
        ...
        try:
            output_text, final_message, generation_seconds, thinking_mode, effort_applied = run_claude_stream(
                anthropic_client,
                anthropic_model,
                exact_payload,
                max_tokens=int(max_tokens),
            ...
            if output_wc < MIN_PANGRAM_WORDS:
                raise RuntimeError(f"Claude returned only {output_wc} words, below Pangram's {MIN_PANGRAM_WORDS}-word minimum.")
    ...
    if last:
        ...
        st.caption(
            f"Claude: {rec['anthropic_model']} · {rec['thinking_mode']} · effort {rec.get('effort') or 'n/a'} · "
    ...
    if not recent_generations.empty:
        with st.expander("Recent Claude → Pangram runs", expanded=False):
            display_cols = [
                "run_at", "prompt_name", "label", "anthropic_model", "thinking_mode", "effort", "target_words", "output_words",
