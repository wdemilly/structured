...
# Fixed audited paragraph selection for the 28-donor controlled v6F build.
# Each tuple is: (P-slot, donor ID, 1-based source paragraph, expected sentence units).
# This map reproduces v6F_28Donor_v1 exactly; it is not recomputed from story content.
CONTROLLED_V6F_28_SELECTION = [
    ("P001", "D021", 2, 6),
    ("P002", "D001", 48, 2),
    ("P003", "D021", 9, 3),
    ("P004", "D016", 2, 2),
...
def donor_id_from_name(name):
    """Return D001..D028 from normalized donor filenames, or None."""
    m = re.match(r"^(D\d{3})(?:__|[_\s-])", name, re.I)
    return m.group(1).upper() if m else None


def controlled_28_donor_selection(docs):
    """
    Select the pre-audited 119 intact paragraphs / 321 sentence units from the
    normalized 28-donor corpus. Input order is irrelevant; donor IDs in the
    filenames are the keys. Returns (chosen_records, errors).
    """
    by_id = {}
    errors = []
    for name, paras in docs:
        did = donor_id_from_name(name)
        if not did:
            errors.append("Cannot identify donor ID from filename: %s" % name)
            continue
        if did in by_id:
            errors.append("Duplicate donor ID %s: %s and %s" % (did, by_id[did][0], name))
            continue
        by_id[did] = (name, paras)

    expected_ids = {"D%03d" % i for i in range(1, 29)}
    found_ids = set(by_id)
    missing = sorted(expected_ids - found_ids)
    extra = sorted(found_ids - expected_ids)
    if missing:
        errors.append("Missing donor IDs: " + ", ".join(missing))
    if extra:
        errors.append("Unexpected donor IDs: " + ", ".join(extra))
    if errors:
        return [], errors

    chosen = []
    for block_id, did, pnum, expected_units in CONTROLLED_V6F_28_SELECTION:
        name, paras = by_id[did]
        if pnum < 1 or pnum > len(paras):
            errors.append("%s: %s paragraph %d does not exist (source has %d paragraphs)." %
                          (block_id, did, pnum, len(paras)))
            continue
        p = paras[pnum - 1]
        got_units = len(sentences(p))
        if got_units != expected_units:
            errors.append("%s: %s paragraph %d has %d sentence units; expected %d." %
                          (block_id, did, pnum, got_units, expected_units))
            continue
        chosen.append(dict(text=p, source=name, source_para=pnum,
                           source_id=did, intended_block_id=block_id,
                           expected_units=expected_units))

    if errors:
        return [], errors
    if len(chosen) != 119 or sum(x[3] for x in CONTROLLED_V6F_28_SELECTION) != 321:
        return [], ["Internal controlled-selection map failed its 119-block / 321-unit invariant."]
    return chosen, []


def controlled_selection_checks(chosen, blocks):
    donor_ids = {r.get("source_id") for r in chosen}
    return [
        ("controlled selection contains exactly 119 source paragraphs", len(chosen) == 119),
        ("controlled selection contains exactly 321 sentence units",
         sum(len(b["units"]) for b in blocks) == 321),
        ("all 28 donor IDs are represented", donor_ids == {"D%03d" % i for i in range(1, 29)}),
        ("canonical P-slot sentence-count vector preserved",
         [len(b["units"]) for b in blocks] == [x[3] for x in CONTROLLED_V6F_28_SELECTION]),
    ]
...
def main():
    ...
    stl.caption("Recreates the v6F structural skeleton format from donor documents. "
    ...
    stl.info(
        ...
        "For the 28-donor experiment, use the controlled build mode below; it selects the pre-audited "
        "119 intact source paragraphs / 321 sentence units before extraction."
    )
    ...
    if not files:
        stl.info("Upload donor documents to begin.")
        return

    names = [f.name for f in files]
    order_names = stl.multiselect("Documents included", names, default=names)
    if not order_names:
    ...
    ids = [donor_id_from_name(n) for n, _ in docs]
    complete_28 = len(docs) == 28 and None not in ids and len(set(ids)) == 28 and set(ids) == {"D%03d" % i for i in range(1, 29)}
    modes = ["Controlled 28-donor v6F build — 119 blocks / 321 units",
             "General extractor — use all selected source paragraphs"]
    build_mode = stl.radio("Build mode", modes, index=0 if complete_28 else 1)

    include_frame = stl.checkbox("Include DONOR CLOSED-CLASS FRAME", value=True,
                                 help="Canonical v6F uses the frame. Leave it off only for a donor set where retaining function-word frames is not appropriate.")

    if build_mode.startswith("Controlled"):
        chosen, selection_errors = controlled_28_donor_selection(docs)
        if selection_errors:
            for e in selection_errors:
                stl.error(e)
            stl.warning("Controlled mode requires exactly the normalized D001–D028 donor set. "
                        "No skeleton will be built until those checks pass.")
            return
        stl.success("Controlled donor set recognized. Build will select exactly 119 intact source paragraphs "
                    "containing 321 sentence units, preserve all 28 donors, and use the canonical Aug. 18 v6F block order.")
        stl.write("The remaining %d source paragraphs are deliberately not used in this controlled build."
                  % (len(para_records) - 119))
        donor_note = "28-donor controlled v6F pool (D001–D028)"
        ...
        if order_mode.startswith("Canonical"):
            ...
            order = None  # built after extraction because block count may differ
            order_note = None
    ...
    if stl.button("Build skeleton", type="primary"):
        with stl.spinner("Extracting…"):
            ...
            if build_mode.startswith("Controlled"):
                build_order = order
                extra_checks = controlled_selection_checks(chosen, blocks)
            else:
                extra_checks = []
                if order is None and 'order_mode' in locals() and order_mode.startswith("Canonical"):
                    stl.error("The canonical Aug. 18 order requires exactly 119 P-blocks. "
                              "Choose a seeded order or use the controlled 28-donor mode.")
                    stl.session_state.pop("result", None)
                    return
                build_order = order if order is not None else block_order(len(blocks), int(render_seed))

            text = render(blocks, build_order, include_frame, donor_note,
                          seed=render_seed, order_note=order_note)
            try:
                docx_bytes = to_docx_bytes(text)
            except ImportError:
                docx_bytes = None
            checks = extra_checks + validate(blocks, build_order, text)
            stl.session_state["result"] = dict(
                ...
                controlled=build_mode.startswith("Controlled"))
    ...
    basename = "Structural_Transplant_Skeleton_v6F_28Donor" if res.get("controlled") else "Structural_Transplant_Skeleton"
    stl.download_button("Download .txt", text.encode("utf8"), key="dl_txt",
                        file_name=basename + ".txt", mime="text/plain")
    stl.download_button("Download provenance .tsv", res["provenance"].encode("utf8"),
                        key="dl_provenance", file_name=basename + "_Provenance.tsv",
    ...
    with stl.expander("ACCURACY — what this tagger does and does not get right"):
        stl.markdown(
            ...
            "**Reconstructed.** PHRASES follows the rule recovered from the surviving v6F skeleton. The exact "
            "lost Aug. 18 parser/tagger source was not preserved."
        )
