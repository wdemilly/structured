...
APP_VERSION = "v2.20 · live activity + delivery mode"
...
# Opus 5 and Sonnet 5 top out at 128k output tokens. v2.18 let the number input
# go to 200,000, which the API rejects. max_tokens also covers thinking, so at
# xhigh/max effort a low cap can be spent entirely on reasoning with no prose left.
CLAUDE_MAX_OUTPUT_TOKENS = 128000
CLAUDE_HIGH_EFFORT_LEVELS = {"xhigh", "max"}
CLAUDE_HIGH_EFFORT_MIN_TOKENS = 100000
...
# claude.ai routes an attachment down two paths at once: the text is embedded in
# the context, and the file itself is written into the code-execution container's
# filesystem via a container_upload block. v2.17 only did the first, so a prompt
# that told Claude to look at its input directory found it empty.
...
WRITE_MODE_NEW = "Write a new chapter"
WRITE_MODE_EXISTING = "Score a chapter I already have"
CLAUDE_DEFAULT_USER_MESSAGE = "write the chapter per this prompt"
CLAUDE_DEFAULT_CHAPTER = "1"
CLAUDE_CONTINUE_MESSAGE = "continue"

# Two ways to get a chapter, and they behave very differently on screen.
#
# REPLICATION obeys whatever the drafting prompt says about delivery. When the
# prompt asks for a .docx, Claude writes the chapter inside a code-execution call
# and the reply is only its commentary, so nothing streams into the transcript
# while the chapter is being written.
#
# INLINE is what v2.14 did: it overrides the prompt's delivery instruction and
# asks for prose in the reply. Text appears immediately because the prose is the
# text stream. It is faster and easier to watch, and it is not a replication of
# the web app.
DELIVERY_REPLICATION = "Follow the prompt's delivery instruction (replication)"
DELIVERY_INLINE = "Prose in the reply (v2.14 behaviour, faster, not replication)"
DELIVERY_MODES = [DELIVERY_REPLICATION, DELIVERY_INLINE]
DELIVERY_INLINE_OVERRIDE = (
    "For this test, this harness overrides any output-format instruction in the attached documents. "
    "Return only the finished chapter prose as plain text in this reply. Do not create a file, and do not "
    "add a title, a preface, an explanation, analysis, notes, or a self-critique."
)
...
REPLICATION_NOTES = [
    ...
    ("Off", "Prose-in-reply override", "Delivery mode is a switch. Set it to the v2.14 behaviour and the app adds an instruction the web app never sends, which breaks the replication in exchange for speed."),
]
...
class Attachment:
    ...
    @property
    def mime(self) -> str:
        return ATTACHMENT_MIME_TYPES.get(Path(self.name).suffix.lower(), "application/octet-stream")


def upload_attachments_to_container(client: Any, attachments: list[Attachment]) -> tuple[list[dict[str, Any]], list[str]]:
    """Upload each attachment to the Files API and return container_upload blocks.

    This is the second of claude.ai's two attachment paths. Without it the code
    execution sandbox starts empty, and a drafting prompt that tells Claude to
    read its input directory finds nothing there.

    Known API behaviour: a container_upload block is silently ignored when no
    container is active, with no error returned. The caller checks the response
    for a container object and warns rather than assuming the mount worked.
    """
    blocks: list[dict[str, Any]] = []
    notes: list[str] = []
    for item in attachments:
        if not item.data:
            notes.append(f"{item.name}: no raw bytes captured, not mounted in the sandbox.")
            continue
        uploaded = None
        last_error: Exception | None = None
        for uploader in (
            lambda: client.beta.files.upload(file=(item.name, io.BytesIO(item.data), item.mime)),
            lambda: client.files.upload(file=(item.name, io.BytesIO(item.data), item.mime)),
        ):
            try:
                uploaded = uploader()
                break
                ...
                last_error = exc
        if uploaded is None:
            notes.append(f"{item.name}: upload failed ({last_error}). It is still in the prompt text, but not on the sandbox filesystem.")
            continue
        blocks.append({"type": "container_upload", "file_id": str(getattr(uploaded, "id", ""))})
    return blocks, notes
...
def build_web_style_request(
    ...
    container_blocks: list[dict[str, Any]] | None = None,
    cache_attachments: bool = True,
    delivery_mode: str = DELIVERY_REPLICATION,
) -> dict[str, Any]:
    system_prompt = render_system_prompt(system_prompt_text, user_preferences)
    container_blocks = list(container_blocks or [])
    documents = build_documents_block(attachments)
    instruction = (user_message or "").strip()
    if delivery_mode == DELIVERY_INLINE:
        # v2.14's override, reinstated only when the writer explicitly asks for it.
        instruction = (instruction + "\n\n" + DELIVERY_INLINE_OVERRIDE).strip()
    user_turn = (documents + "\n\n" + instruction).strip() if documents and instruction else (documents or instruction)

    content: Any = user_turn
    if container_blocks or (cache_attachments and documents):
        blocks: list[dict[str, Any]] = list(container_blocks)
        if cache_attachments and documents:
            # A cache breakpoint on the documents only. The instruction varies
            # between runs, so keeping it in its own block leaves the expensive
            # prefix byte-identical and re-usable. Caching changes cost and
            # latency, not what the model produces.
            blocks.append({
                "type": "text",
                "text": documents,
            ...
            blocks.append({"type": "text", "text": user_turn})
        content = blocks
    preview_parts = []
    if system_prompt:
    ...
    if container_blocks:
        preview_parts.append(
            "USER — sandbox mounts\n"
            + "\n".join(f"container_upload · {b['file_id']}" for b in container_blocks)
        )
...
def run_web_style_turn(
    ...
    *,
    ...
    for attempt in range(1, CLAUDE_MAX_TOOL_CONTINUATIONS + 1):
        ...
        # v2.14 streamed prose because prose was the whole reply. Once the prompt's
        # own "deliver a .docx" instruction is obeyed, the chapter is written inside
        # a tool call and no text arrives until the very end. Painting only text
        # deltas therefore leaves the screen blank for minutes. Walk the raw events
        # instead so thinking and tool activity are visible while they happen.
        last_paint = 0.0
        thinking_chars = 0
        ...
        def _paint(force: bool = False) -> None:
            ...
            if not force and now - last_paint < 0.5:
                return
            last_paint = now
            elapsed = now - started
            bits = [f"`{phase}`", f"{elapsed:,.0f}s"]
            if thinking_chars:
                bits.append(f"~{thinking_chars:,} chars of thinking")
            if tool_calls_local[0]:
                bits.append(f"{tool_calls_local[0]} tool call(s)")
            body = "".join(text_chunks)
            live_placeholder.markdown(" · ".join(bits) + (("\n\n" + body) if body else ""))
            ...
            for event in stream:
                ...
                if etype == "content_block_start":
                    ...
                    if btype == "thinking":
                        ...
                        tool_calls_local[0] += 1
                        phase = "running code / writing the file"
                    elif btype.endswith("tool_result"):
                        phase = "reading the tool result"
                    elif btype == "text":
                    ...
                    if dtype == "text_delta":
                        text_chunks.append(str(getattr(delta, "text", "") or ""))
                    elif dtype == "thinking_delta":
                        thinking_chars += len(str(getattr(delta, "thinking", "") or ""))
                    _paint()
            final_message = stream.get_final_message()
        tool_calls += tool_calls_local[0]

        usage = claude_usage_details(final_message)
        input_tokens += int(usage["input_tokens"] or 0)
        output_tokens += int(usage["output_tokens"] or 0)
        thinking_tokens += int(usage["thinking_tokens"] or 0)
        cache_read_total += int(usage.get("cache_read") or 0)
        cache_write_total += int(usage.get("cache_write") or 0)
        ...
        container_obj = getattr(final_message, "container", None)
        if container_obj is not None and container_id is None:
            container_id = str(getattr(container_obj, "id", "") or "")
...
# -------------------------
with claude_tab:
    st.subheader("Write a chapter, or score one you already have")

    # Project and chapter identify the run in both modes, so they sit above the split.
    id1, id2 = st.columns([2.0, 1.0])
    with id1:
        project_name = st.text_input(
            "Project",
            value="",
            key="write_project_v217",
            placeholder="Book or working title. Stored with the run so history can be filtered by book.",
        )
    with id2:
        chapter_id = st.text_input(
            "Chapter",
            value=CLAUDE_DEFAULT_CHAPTER,
            key="write_chapter_v217",
            help="Whatever you call it. Free text; nothing parses this.",
        )

    write_mode = st.radio(
        "What do you want to do?",
        [WRITE_MODE_NEW, WRITE_MODE_EXISTING],
        horizontal=True,
        key="write_mode_v217",
    )
    ...
    # =====================================================================
    # Mode B: score a chapter that already exists as a file
    # =====================================================================
    if write_mode == WRITE_MODE_EXISTING:
        st.write(
            "Upload a finished chapter — a hand run in claude.ai, an older draft, anything — and score it "
            "through exactly the same Pangram path this tab uses for its own runs. This is how you get a "
            "ground-truth number to compare a replication run against."
        )
        ...
        ext_upload = st.file_uploader(
            ...
            key="write_external_upload_v217",
            help="DOCX, TXT, or MD. The whole file is treated as the chapter.",
        )
        ...
        if ext_upload is not None:
            try:
                ...
                st.success(
                    f"**{ext_upload.name}** loaded — {count_words(ext_text):,} words, "
                    f"{len(split_sentences(ext_text)):,} sentence units."
                )
                with st.expander("Preview what will be scored", expanded=False):
                    st.text_area(
                        "Chapter text",
                        value=ext_text,
                        height=280,
                        disabled=True,
                        key=f"write_external_preview_v217_{ext_upload.name}",
                    )
            except Exception as exc:
        ...
        ext_provenance = st.text_input(
            ...
            value="",
            key="write_external_provenance_v217",
            placeholder="e.g. claude.ai hand run, Opus 5, max effort, R4.1 instrument + 22.6 packet",
            help="Free text, stored with the run so the comparison stays interpretable months from now.",
        )
        ...
        if ext_upload is None:
            st.info("Choose a chapter file to score.")

        ext_ready = bool(ext_text.strip() and pangram_client and pangram_model)
        if st.button(
            "Score this chapter",
            type="primary",
            disabled=not ext_ready,
            key="write_external_score_v217",
        ):
            ...
            try:
                ...
                save_results(
                    ext_rows,
                    experiment_name=f"{project_name or 'external'} ch {chapter_id}",
                ...
                save_generation_run({
                    "run_id": ext_run_id,
                    "run_at": datetime.now(timezone.utc).isoformat(),
                    "label": f"{project_name or 'external'} ch {chapter_id} (external)",
                ...
                st.session_state["write_external_last_v217"] = {
        ...
        ext_last = st.session_state.get("write_external_last_v217")
        if ext_last:
            es = ext_last["summary"]
            st.divider()
            st.markdown("### Result")
            ...
            show_result_table(results_dataframe(ext_last["rows"]), key="write_external_table_v217")
            ...
            st.download_button(
                ...
                mime="text/csv",
                key=f"write_external_csv_v217_{ext_last['run_id']}",
            )

    # =====================================================================
    # Mode A: write a new chapter the way claude.ai would
    # =====================================================================
    else:
        st.write(
            "Attach the same files you would attach in the web app, type the same one-line instruction, and run. "
            "This sends the published claude.ai system prompt, puts your attachments above the instruction, "
        ...
        with a1:
            prompt_upload = st.file_uploader(
                ...
                type=["docx", "txt", "md"],
                accept_multiple_files=False,
                key="write_prompt_upload_v217",
                help="The instrument document, e.g. a structural transplant prompt.",
            )
        with a2:
            packet_upload = st.file_uploader(
                ...
                type=["docx", "txt", "md"],
                accept_multiple_files=False,
                key="write_packet_upload_v217",
                help="The per-chapter packet. It supplies the chapter and its word target.",
            )
        extra_uploads = st.file_uploader(
            "Any other files to attach (optional)",
            type=["docx", "txt", "md"],
            accept_multiple_files=True,
            key="write_extra_uploads_v217",
            help="Attached after the two above, in the order you add them.",
        )

        attachments: list[Attachment] = []
        for upload in [prompt_upload, packet_upload] + list(extra_uploads or []):
            if upload is None:
                continue
            try:
        ...
        if attachments:
            st.caption(
                ...
                + " → ".join(f"{a.name} ({count_words(a.text):,}w)" for a in attachments)
                + f" · {sum(count_words(a.text) for a in attachments):,} words total."
            )
        ...
        message_template = st.text_input(
            ...
            key="write_message_template_v218",
            help=(
                "Exactly what you would type in the app. {chapter} is substituted from the Chapter box, "
                "but most drafting prompts expect the packet to name the chapter, so the default says nothing about it."
            ),
        )
        try:
        ...
        # A message that names a chapter the packet does not is the fastest way to
        # get a refusal instead of a draft, because the drafting prompt treats the
        # packet as the only authority on which chapter this is.
        asserted = re.findall(r"\bch(?:apter)?\.?\s*([0-9]{1,3})\b", user_message, flags=re.I)
        if asserted and attachments:
            packet_blob = "\n".join(a.text for a in attachments)
            found = set(re.findall(r"\bchapter\s*([0-9]{1,3})\b", packet_blob, flags=re.I))
            if found and not (set(asserted) & found):
                st.error(
                    f"Your message says chapter {', '.join(sorted(set(asserted)))}, but the attachments only "
                    f"mention chapter {', '.join(sorted(found, key=int))}. Claude will refuse rather than invent "
                    "a chapter the packet does not describe. Fix the message or attach the matching packet."
                )
        ...
        if stored_source is None:
            stored_source = load_setting(
                SYSTEM_PROMPT_SETTING_KEY + "_source",
                "Embedded snapshot: Claude Opus 5, July 24 2026",
            )
        ...
        with st.expander("claude.ai system prompt", expanded=False):
            ...
            with f1:
                fetch_slug = st.text_input("Docs slug", value="claude-opus-5", key="write_slug_v217")
            with f2:
                st.write("")
                if st.button("Fetch published prompt", key="write_fetch_prompt_v217", use_container_width=True):
                    try:
            ...
            system_prompt_text = st.text_area(
                ...
                height=300,
                key="write_system_prompt_area_v217",
            )
        ...
        delivery_mode = st.radio(
            "How should the chapter come back?",
            DELIVERY_MODES,
            index=0,
            key="write_delivery_v220",
            help=(
                "Replication obeys the drafting prompt. If it asks for a .docx, Claude writes the chapter inside a "
                "code-execution call, so nothing appears in the transcript until it finishes. The v2.14 behaviour "
                "adds an instruction the web app never sends, asking for prose in the reply: text starts appearing "
                "at once and the run is quicker, but it is no longer a replication."
            ),
        )
        if delivery_mode == DELIVERY_INLINE:
            st.info(
                "Prose-in-reply is on. This is the v2.14 harness override, useful for fast craft iteration. "
                "Runs made this way should not be compared against a claude.ai hand run."
            )

        if st.session_state.get("write_delivery_stamp_v220") != delivery_mode:
            st.session_state["write_delivery_stamp_v220"] = delivery_mode
            st.session_state["write_code_exec_v220"] = delivery_mode == DELIVERY_REPLICATION
        ...
        with m1:
            if tab_models:
                ...
                selected_model = None
                st.selectbox("Claude model", ["Connect Anthropic first"], disabled=True, key="write_model_disabled_v217")
        with m2:
            effort = st.selectbox(
                "Effort",
                CLAUDE_EFFORT_LEVELS,
                index=CLAUDE_EFFORT_LEVELS.index(CLAUDE_DEFAULT_EFFORT),
                key="write_effort_v217",
        ...
        with m3:
            max_tokens = st.number_input(
                "Max output tokens",
                min_value=1000,
                max_value=CLAUDE_MAX_OUTPUT_TOKENS,
                value=CLAUDE_DEFAULT_MAX_TOKENS,
                step=1000,
                key="write_max_tokens_v219",
            )
        st.caption(
            f"Covers thinking and visible output together, and Opus 5 tops out at {CLAUDE_MAX_OUTPUT_TOKENS:,}. "
            "A truncated run is reported rather than scored."
        )
        if effort in CLAUDE_HIGH_EFFORT_LEVELS and int(max_tokens) < CLAUDE_HIGH_EFFORT_MIN_TOKENS:
            st.warning(
                f"Effort **{effort}** with a {int(max_tokens):,}-token cap is the combination that returns an empty "
                "reply: thinking counts against this budget, and at this effort Claude can spend all of it reasoning "
                f"before writing a word. Raise the cap toward {CLAUDE_MAX_OUTPUT_TOKENS:,}, or drop to high."
            )
        ...
        with x1:
            enable_code_execution = st.checkbox(
                ...
                disabled=delivery_mode == DELIVERY_INLINE,
                key="write_code_exec_v220",
                help=(
        ...
        with x2:
            mount_attachments = st.checkbox(
                "Mount attachments in the sandbox",
                value=True,
                key="write_mount_v218",
                disabled=not enable_code_execution,
                help=(
                    "claude.ai puts an attachment in two places at once: the text in the conversation, and the file "
                    "itself on the code-execution filesystem. This uploads them via the Files API so a prompt that "
                    "looks at its input directory finds them there."
                ),
            )
        enable_code_execution = bool(enable_code_execution and delivery_mode == DELIVERY_REPLICATION)
        mount_attachments = bool(mount_attachments and enable_code_execution)

        cache_attachments = st.checkbox(
            "Cache the attachments between runs",
            value=True,
            key="write_cache_v219",
            help=(
                "Puts a prompt-cache breakpoint on the attachment text so a repeat run of the same instrument "
                "is billed and served from cache instead of re-reading it. Affects cost and latency only, not "
                "what Claude writes. The cache is short-lived, so back-to-back runs benefit most."
            ),
        )
        ...
        if mount_attachments and attachments and anthropic_client:
            stamp = json.dumps([[a.name, len(a.data)] for a in attachments])
            cached = st.session_state.get("write_container_cache_v218")
            if cached and cached.get("stamp") == stamp:
                container_blocks = cached["blocks"]
            else:
                with st.spinner("Uploading attachments to the sandbox…"):
                    container_blocks, mount_notes = upload_attachments_to_container(anthropic_client, attachments)
                st.session_state["write_container_cache_v218"] = {"stamp": stamp, "blocks": container_blocks}
                for note in mount_notes:
                    st.warning(note)
            if container_blocks:
                st.caption(f"{len(container_blocks)} file(s) mounted in the code-execution sandbox.")
        ...
        if attachments and user_message.strip():
            request_bundle = build_web_style_request(
                attachments,
                user_message,
                system_prompt_text if st.session_state.get("write_send_system_v217", True) else "",
                user_preferences,
                container_blocks=container_blocks,
        ...
        def _score_and_store(chapter_text: str, turn: dict[str, Any], resolved: dict[str, Any], *, turn_index: int) -> None:
            ...
            # The drafting prompt declares the delivery contract. If it demanded a
            # file and no file came back, the reply is Claude talking about the run,
            # not the chapter, and scoring it writes a meaningless row into history.
            demands_file = bool(re.search(r"\.docx|\bword\s+file\b|\bword\s+document\b", attachment_text, flags=re.I))
            if demands_file and resolved["chapter_source"].startswith("assistant reply"):
                raise RuntimeError(
                    "The drafting prompt asks for the chapter as a file, but no file was created, so this reply "
                    "is Claude talking rather than the chapter. Nothing was scored. "
                    + (
                        "The sandbox had no container this turn, so mounted attachments may not have arrived. "
                        if not turn.get("container_id") else ""
                    )
                    + "The reply is shown above."
                )
            ...
            record = {
                ...
                "cache_write_tokens": turn.get("cache_write"),
                ...
                "provenance": (
                    f"generated in app · {delivery_mode} · container {turn.get('container_id') or 'none'}"
                ),
        ...
        with run_col:
            run_clicked = st.button(
                "Send", type="primary", disabled=not ready, key="write_run_v217",
                use_container_width=True,
            )
        with cont_col:
        ...
        if run_clicked or continue_clicked:
            ...
            try:
                ...
                if request_bundle.get("container_blocks") and not turn.get("container_id"):
                    st.warning(
                        "Attachments were sent as sandbox mounts, but the API returned no container for this turn. "
                        "A container_upload block is silently dropped when no container is active, so Claude may "
                        "have seen an empty input directory. If the reply says files are missing, that is why."
                    )

                if turn["stop_reason"] == "max_tokens":
                    spent_thinking = int(turn.get("thinking_tokens") or 0)
                    spent_total = int(turn.get("output_tokens") or 0)
                    share = f"{100*spent_thinking/spent_total:.0f}%" if spent_total else "an unknown share"
                    st.warning(
                        f"The reply hit the {int(max_tokens):,}-token cap. Of {spent_total:,} output tokens, "
                        f"{spent_thinking:,} ({share}) went to thinking, which counts against the same budget. "
                        + (
                            f"No prose was produced at all, so effort **{effort}** used the entire budget reasoning. "
                            f"Drop to high, or raise the cap toward {CLAUDE_MAX_OUTPUT_TOKENS:,}."
                            if not turn.get("reply_text", "").strip()
                            else "The text below is truncated; raise the cap or send continue before scoring it."
                        )
                    )
        ...
        if last:
            ...
            st.caption(
                ...
                f"{rec.get('thinking_tokens') or 0:,} thinking · "
                f"{rec.get('cache_read_tokens') or 0:,} cached · stop reason: {rec.get('stop_reason') or 'n/a'}."
            )
            st.write(
            ...
            with st.expander("Chapter", expanded=True):
                st.text_area(
                    ...
                    height=420,
                    disabled=True,
                    key=f"write_chapter_out_v217_{rec['run_id']}",
                )
            ...
            with d1:
                ...
                st.download_button(
                    ...
                    key=f"write_docx_v217_{rec['run_id']}",
                    use_container_width=True,
                )
            ...
            with d2:
                ...
                st.download_button(
                    ...
                    key=f"write_json_v217_{rec['run_id']}",
                    use_container_width=True,
                )

            st.markdown("**Pangram 150-word windows**")
            show_result_table(results_dataframe(last["rows"]), key="write_pangram_table_v217")
            ...
            if st.button("Start a new conversation", key=f"write_reset_v217_{rec['run_id']}"):
                st.session_state["write_last_v215"] = None
    ...
    if not recent.empty:
        with st.expander("Recent runs in this tab", expanded=False):
            display_cols = [
