...
APP_VERSION = "v2.6 · explicit Claude model + effort · adaptive thinking"
...
with st.sidebar:
    ...
    if amodels:
        current = st.session_state.get("anthropic_model")
        aidx = amodels.index(current) if current in amodels else 0
        aselected = st.selectbox("Claude model", amodels, index=aidx, key="anthropic_model_select")
        st.session_state["anthropic_model"] = aselected
        st.caption("Read live from the Models API for your Anthropic key.")
...
with claude_tab:
    ...
    with c1:
        run_label = st.text_input("Run label", value="rapid-test", key="claude_run_label_v26")
    with c2:
        target_words = st.number_input(
            ...
            step=50,
            key=f"claude_target_words_v26_{target_default}",
        )
    with c3:
        effort = st.selectbox(
            "Claude effort",
