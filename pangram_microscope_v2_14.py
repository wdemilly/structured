...
APP_VERSION = "v2.14 · three-part API continuation"
...
CLAUDE_GENERATION_SINGLE = "Single request"
CLAUDE_GENERATION_THREE_PART = "3-part chained conversation"
CLAUDE_GENERATION_STRATEGIES = [CLAUDE_GENERATION_SINGLE, CLAUDE_GENERATION_THREE_PART]
...
def build_claude_test_message(
    prompt_text: str,
    packet_text: str,
    target_words: int,
    generation_strategy: str = CLAUDE_GENERATION_SINGLE,
) -> str:
    """Create one fixed, auditable API payload. No prior runs or detector feedback enter it."""
    if generation_strategy == CLAUDE_GENERATION_THREE_PART:
        run_instruction = (
            f"Begin the three-pass generation now by executing PASS 1 only. For this rapid test, the full chapter target is "
            f"approximately {int(target_words)} words; do not turn that into a per-pass quota. Follow the drafting prompt's own "
            "pass-boundary and continuation-state rules exactly. For this API test only, override any DOCX/file-creation instruction: "
            "return the newly generated Pass 1 fiction prose as plain text, followed by the prompt's required CONTINUATION STATE line. "
            "Do not begin Pass 2."
        )
    else:
        run_instruction = (
...
def build_claude_request(
    ...
    Standard mode preserves the v2.10 wrapper. Parity modes keep the prompt and packet
    in separate content blocks. Three-part mode changes only the run instruction: the
    initial request executes Pass 1; subsequent API turns are exactly CONTINUE.
    """
    if harness_mode == CLAUDE_HARNESS_STANDARD:
        payload = build_claude_test_message(
            prompt_text, packet_text, int(target_words), generation_strategy=generation_strategy
        )
        return {
            "messages": [{"role": "user", "content": payload}],
            "system": None,
            "description": (
                "Standard v2.10 wrapper · 3-part chained"
                if generation_strategy == CLAUDE_GENERATION_THREE_PART
                else "Standard v2.10 wrapper"
            ),
            "preview": payload,
        }
    ...
    if generation_strategy == CLAUDE_GENERATION_THREE_PART:
        final_instruction = (
            "Begin the three-pass generation now by executing PASS 1 only. Follow the drafting prompt's own "
            "pass-boundary and continuation-state rules exactly. For this API test only, override any DOCX/file-creation "
            "instruction: return the newly generated Pass 1 fiction prose as plain text, followed by the prompt's required "
            "CONTINUATION STATE line. Do not begin Pass 2."
        )
    else:
        final_instruction = (
    ...
    if generation_strategy == CLAUDE_GENERATION_THREE_PART:
        preview_parts.append("FOLLOW-UP USER TURN AFTER PASS 1\nCONTINUE")
        preview_parts.append("FOLLOW-UP USER TURN AFTER PASS 2\nCONTINUE")
...
def split_api_part_output(text: str) -> tuple[str, str | None]:
    """Remove v6FX mechanical continuation-state lines from the fiction to be scored."""
    state_line = None
    prose_lines: list[str] = []
    normalized = (text or "").replace("\r\n", "\n").replace("\r", "\n")
    for line in normalized.split("\n"):
        stripped = line.strip()
        if stripped.upper().startswith("CONTINUATION STATE:"):
            state_line = stripped
            continue
        prose_lines.append(line)
    return clean_text("\n".join(prose_lines)), state_line


def run_claude_three_part_stream(
    client: Any,
    model: str,
    ...
    *,
    max_tokens: int,
    effort: str,
    live_placeholders: list[Any],
    system_prompt: str | None = None,
) -> dict[str, Any]:
    """Run three sequential Messages API calls as one conversation.

    The assistant's raw prior response is carried forward verbatim. The only new user
    content between parts is exactly CONTINUE. Continuation-state lines remain in the
    conversation for auditability but are stripped from the fiction concatenated for Pangram.
    """
    messages = list(initial_messages)
    raw_parts: list[str] = []
    prose_parts: list[str] = []
    continuation_states: list[str | None] = []
    ...
    for part_no in range(1, 4):
        output_text, final_message, elapsed, generation_mode = run_claude_stream(
            client,
            model,
            messages,
            max_tokens=int(max_tokens),
            effort=effort,
            live_placeholder=live_placeholders[part_no - 1],
        ...
        input_tokens_total += int(usage["input_tokens"] or 0)
        output_tokens_total += int(usage["output_tokens"] or 0)
        thinking_tokens_total += int(usage["thinking_tokens"] or 0)

        if stop_reason == "refusal":
            detail_bits = [x for x in [refusal_category, refusal_explanation] if x]
            raise RuntimeError(
                f"Claude refused Pass {part_no}: " + (" · ".join(map(str, detail_bits)) or "no details supplied")
            )
        if stop_reason == "max_tokens":
            raise RuntimeError(
                f"Claude hit the {int(max_tokens):,}-token cap during Pass {part_no}. "
                "The continuation cursor may be incomplete, so the chained run was stopped rather than scored."
            )

        prose_text, state_line = split_api_part_output(output_text)
        if not prose_text.strip():
            raise RuntimeError(f"Pass {part_no} returned no fiction prose.")
        raw_parts.append(output_text)
        prose_parts.append(prose_text)
        continuation_states.append(state_line)

        if part_no < 3:
            messages.append({"role": "assistant", "content": output_text})
            messages.append({"role": "user", "content": "CONTINUE"})

    combined_prose = clean_text("\n\n".join(p for p in prose_parts if p.strip()))
    return {
        "output_text": combined_prose,
        "raw_parts": raw_parts,
        "prose_parts": prose_parts,
        "part_word_counts": [count_words(p) for p in prose_parts],
...
st.caption(
    "Generate a clean Claude API sample in single-request or chained three-part mode and score it with Pangram, "
    "while keeping the existing calibration, prompt-experiment, and structural-diversity tools."
)
...
# -------------------------
with claude_tab:
    st.subheader("Claude API fiction test")
    st.write(
        "Upload the drafting prompt and chapter packet, choose the API harness, then choose a single API request or a "
        "three-part chained conversation. Single-request runs remain stateless. In three-part mode, only the earlier parts "
        "of the same run are carried forward; prior runs and Pangram feedback are never included in generation."
    )
    st.info(
    ...
    generation_strategy = st.radio(
        "Generation strategy",
        CLAUDE_GENERATION_STRATEGIES,
        index=0,
        horizontal=True,
        key="claude_generation_strategy_v214",
        help=(
            "Single request preserves v2.13 behavior. Three-part chained conversation makes three sequential API calls. "
            "After Pass 1 and Pass 2, the prior assistant response is carried forward verbatim and the only new user turn is CONTINUE."
        ),
    )
    if generation_strategy == CLAUDE_GENERATION_THREE_PART:
        st.info(
            "Three-part mode is intended for a prompt that defines its own three-pass behavior, such as 6FX. "
            "The app does not pre-split the packet or invent new pass boundaries."
        )
        if prompt_text.strip() and not re.search(r"three[- ]pass|pass\s+1|continuation state", prompt_text, flags=re.I):
            st.warning(
                "This prompt does not appear to contain explicit three-pass instructions. The chained API mode will still run, "
                "but for a controlled experiment use a prompt such as 6FX that defines the pass boundaries and cursor rules."
            )
    ...
    with g3:
        max_tokens = st.number_input(
            ...
            step=1000,
            key="claude_max_tokens_v213",
        )
        st.caption("This cap covers Claude's hidden thinking + visible prose. 64k is the recommended starting cap for the v6F parity test at High effort.")
    ...
    if prompt_text and packet_text:
        ...
        with st.expander("Exact Claude request content", expanded=False):
            ...
            if generation_strategy == CLAUDE_GENERATION_THREE_PART:
                st.caption(
                    "Within this run, Pass 1 and Pass 2 assistant outputs are carried forward verbatim and each follow-up user turn is exactly CONTINUE. "
                    "No prior run and no Pangram feedback is appended. In Parity A/B, the target-word control is logged but is not injected into the prompt."
                )
            else:
                st.caption(
    ...
    run_button_label = (
        "Generate 3 parts with Claude and score combined chapter"
        if generation_strategy == CLAUDE_GENERATION_THREE_PART
        else "Generate with Claude and score with Pangram"
    )
    if st.button(
        run_button_label,
        type="primary",
        disabled=not ready,
        key="claude_generate_score_v214",
    ):
        ...
        try:
            ...
            if generation_strategy == CLAUDE_GENERATION_THREE_PART:
                part_live = []
                for part_no in range(1, 4):
                    st.markdown(f"**Claude Pass {part_no}**")
                    part_live.append(st.empty())
                chained = run_claude_three_part_stream(
                    anthropic_client,
                    anthropic_model,
                    request_bundle["messages"],
                    max_tokens=int(max_tokens),
                    effort=effort,
                    live_placeholders=part_live,
                    system_prompt=request_bundle.get("system"),
                )
                output_text = chained["output_text"]
                part_outputs = chained["raw_parts"]
                part_word_counts = chained["part_word_counts"]
                continuation_states = chained["continuation_states"]
                generation_seconds = float(chained["generation_seconds"])
                generation_mode = str(chained["generation_mode"])
                input_tokens = int(chained["input_tokens"])
                output_tokens = int(chained["output_tokens"])
                thinking_tokens = int(chained["thinking_tokens"])
                stop_reason = " | ".join(chained["stop_reasons"])
                refusal_category = None
                refusal_explanation = None
                missing_states = [str(i + 1) for i, s in enumerate(continuation_states[:2]) if not s]
                if missing_states:
                    st.warning(
                        "The chained run completed, but no CONTINUATION STATE line was detected after Pass "
                        + ", ".join(missing_states)
                        + ". The actual prior assistant response was still carried forward, so CONTINUE had conversational context."
                    )
            else:
                live = st.empty()
            ...
            if output_wc < MIN_PANGRAM_WORDS:
                if stop_reason == "max_tokens":
                    thinking_note = (
                        f" ({thinking_tokens:,} were thinking tokens)"
                        if isinstance(thinking_tokens, int) else ""
                    )
                    raise RuntimeError(
                        f"{anthropic_model} exhausted the {int(max_tokens):,}-token output budget before producing usable prose. "
                        f"The API reports {output_tokens or 0:,} output tokens{thinking_note}. "
                        "For adaptive thinking, max_tokens covers hidden reasoning and visible prose together. "
                        "Raise Max output tokens (64,000 is the recommended starting point for this parity test) and rerun."
                    )
                raise RuntimeError(
            ...
            run_status.update(label="Claude generation complete. Pangram scoring combined prose…", state="running")
            ...
            record = {
                ...
                "thinking_mode": f"{generation_mode} · {harness_mode} · {generation_strategy}",
                ...
                "part_count": 3 if generation_strategy == CLAUDE_GENERATION_THREE_PART else 1,
                "part_word_counts": json.dumps(part_word_counts, ensure_ascii=False),
                "continuation_states": json.dumps(continuation_states, ensure_ascii=False),
    ...
    if last:
        ...
        if rec.get("input_tokens") is not None or rec.get("output_tokens") is not None:
            thinking_part = (
                f" · {rec.get('thinking_tokens'):,} thinking tokens"
                if isinstance(rec.get('thinking_tokens'), int) else ""
            )
            st.caption(
                f"Anthropic usage: {rec.get('input_tokens') or 0:,} input tokens · "
                f"{rec.get('output_tokens') or 0:,} output tokens{thinking_part} · "
                f"stop reason: {rec.get('stop_reason') or 'n/a'}."
            )
        st.write(
        ...
        if rec.get("generation_strategy") == CLAUDE_GENERATION_THREE_PART:
            counts = last.get("part_word_counts") or []
            if counts:
                st.caption("Three-part API output: " + " · ".join(f"Pass {i+1}: {wc:,} words" for i, wc in enumerate(counts)))
            for i, raw_part in enumerate(last.get("part_outputs") or [], start=1):
                with st.expander(f"Raw API Pass {i} output", expanded=False):
                    st.text_area(
                        f"Pass {i}",
                        value=raw_part,
                        height=300,
                        disabled=True,
                        key=f"claude_part_output_v214_{rec.get('run_id', 'unknown')}_{i}",
                    )
        ...
        output_heading = "Combined generated prose" if rec.get("generation_strategy") == CLAUDE_GENERATION_THREE_PART else "Generated prose"
        with st.expander(output_heading, expanded=True):
